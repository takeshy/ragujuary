...
Gemini Embedding API は、テキストのベクトル表現を生成し、カスタムセマンティック検索を可能にします：
...
Gemini Embedding API を使ってテキストコンテンツをインデックスし、ローカルセマンティック検索を可能にします。

| パラメータ | 型 | 必須 | 説明 |
|-----------|------|------|-------------|
| `store_name` | string | はい | エンベディングストアの名前 |
| `file_name` | string | はい | ファイル名または識別子 |
| `file_content` | string | はい | インデックスするテキストコンテンツ |
...
- 入力トークン上限: 8,192
- 出力次元数: 128〜3,072
- モデル間のエンベディング空間は互換性なし（モデル変更時は全再インデックス必要）
